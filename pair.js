...
//=======================================
                case 'ping': {     
                    ...
                    let ping = await socket.sendMessage(sender, { text: '*_Pinging to Arslan-Mini-Bot Module..._* ❗' });
                ...
                case 'owner': {
                    const vcard = 'BEGIN:VCARD\n'
                        ...
                        + 'EMAIL:arslanmd@gmail.com\n'
                        + 'END:VCARD';

                    await socket.sendMessage(sender, {
                        contacts: {
                            displayName: "ARSLAN-MD OWNER",
                            contacts: [{ vcard }]
                        },
                        image: { url: config.BUTTON_IMAGES.OWNER },
                        caption: '*👨‍💻 ARSLAN BOT OWNER DETAILS*',
                        buttons: [
                            { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📋 MENU' }, type: 1 },
                ...
                case 'song': {
                    try {
                        ...
                        const titleText = '*ARSLAN-MINI-BOT-SONG DOWNLOAD*';
                        const content = `┏━━━━━━━━━━━━━━━━\n` +
                            `┃📝 \`Title\` : ${video.title}\n` +
                            `┃📈 \`Views\` : ${video.views}\n` +
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                try {
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'Arslan-Md-Free-Bot-Session'}`);
                }
            }
        });
    } catch (error) {
